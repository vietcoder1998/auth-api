...
export async function searchAllEntities(req: Request, res: Response) {
    ...
    try {
        ...
        const results: SearchResult[] = [
            ...users.map((u): UserResult => ({
                type: 'user',
                name: u.nickname || u.email || '',
                description: u.email || '',
                id: u.id,
            })),
            ...roles.map((r): RoleResult => ({
                type: 'role',
                name: r.name || '',
                description: r.description ?? '',
                id: r.id,
            })),
            ...permissions.map((p): PermissionResult => ({
                type: 'permission',
                name: p.name || '',
                description: p.description ?? '',
                id: p.id,
            })),
            ...agents.map((a): AgentResult => ({
                type: 'agent',
                name: a.name || '',
                description: a.description ?? '',
                id: a.id,
            })),
            ...conversations.map((c): ConversationResult => ({
                type: 'conversation',
                name: c.title || '',
                description: c.summary ?? '',
                id: c.id,
            })),
            ...mailTemplates.map((m): MailTemplateResult => ({
                type: 'mailTemplate',
                name: m.name || '',
                description: m.subject ?? '',
                id: m.id,
            })),
            ...notificationTemplates.map((n): NotificationTemplateResult => ({
                type: 'notificationTemplate',
                name: n.name || '',
                description: n.title ?? '',
                id: n.id,
            })),
            ...apiKeys.map((a): ApiKeyResult => ({
                type: 'apiKey',
                name: a.name || '',
                description: a.description ?? '',
                id: a.id,
            })),
            ...dbConnections.map((d): DatabaseConnectionResult => ({
                type: 'databaseConnection',
                name: d.name || '',
